...
/* package */ final class FfmpegDecoder extends
  ...
  protected DecoderException decode(
    ...
    if (result < 0) {
      return new DecoderException("Error decoding (see logcat). Code: " + result);
    }
    if (!hasOutputFormat) {
  ...
  /**
   * Returns the channel count of output audio. May only be called after {@link #}.
   */
  ...
  /**
   * Returns the sample rate of output audio. May only be called after {@link #}.
   */
